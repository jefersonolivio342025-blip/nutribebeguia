...
<span className="text-xs text-muted-foreground mt-1 uppercase">Horas</span>
</div>
<span className="text-2xl font-bold text-destructive animate-pulse">:</span>
<div className="flex flex-col items-center">
<div className="bg-destructive text-destructive-foreground text-2xl sm:text-3xl font-black px-3 sm:px-4 py-2 rounded-xl min-w-[60px] sm:min-w-[70px] text-center shadow-lg">
{formatNumber(timeLeft.minutes)}
</div>
<span className="text-xs text-muted-foreground mt-1 uppercase">Min</span>
</div>
<span className="text-2xl font-bold text-destructive animate-pulse">:</span>
<div className="flex flex-col items-center">
<div className="bg-destructive text-destructive-foreground text-2xl sm:text-3xl font-black px-3 sm:px-4 py-2 rounded-xl min-w-[60px] sm:min-w-[70px] text-center shadow-lg">
...
<span className="text-xs text-muted-foreground mt-1 uppercase">Seg</span>
</div>
</div>
);
};
...
useEffect(() => {
const timer = setInterval(() => {
setSoldCount((prev) => (prev >= 987 ? 847 : prev + 1));
}, 45000);
return () => clearInterval(timer);
}, []);
...
<span className="text-sm font-semibold text-foreground">🔥 {soldCount} usuários ativos</span>
<span className="text-sm text-destructive font-bold">Restam {totalUnits - soldCount} vagas!</span>
</div>
